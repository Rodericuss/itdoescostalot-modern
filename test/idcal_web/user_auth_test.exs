defmodule IdcalWeb.UserAuthTest do
  use IdcalWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Idcal.Accounts
  alias Idcal.Accounts.Scope
  alias IdcalWeb.UserAuth

  import Idcal.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, IdcalWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      IdcalWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        IdcalWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
